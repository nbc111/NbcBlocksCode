...
export function yoctoToPex(yocto: string, format: boolean) {
...
export function gasFee(gas: string, price: string) {
  const near = yoctoToPex(Big(gas).mul(Big(price)).toString(), true);
...
export function convertAmountToReadableString(amount: string, type: string) {
  ...
  const amountInPex = new Big(amount).div(nearNomination);

  if (type === 'totalSupply' || type === 'totalStakeAmount') {
    value = formatWithCommas(amountInPex.div(1e6).toFixed(1));
    suffix = 'M';
  } else if (type === 'seatPriceAmount') {
    value = formatWithCommas(amountInPex.round().toString());
  } else {
