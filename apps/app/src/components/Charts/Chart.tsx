...
import { yoctoToPex } from '@/utils/libs';
...
const Chart = (props: Props) => {
  ...
  const charts = [
    {
      ...
      text: t ? t('charts:nearPrice.heading') : 'Nbc Daily Price (USD) Chart',
    ...
    },
    {
      ...
      text: t
        ...
        : 'Nbc Market Capitalization Chart',
    ...
    },
    {
      ...
      text: t ? t('charts:nearSupply.heading') : 'Nbc Supply Growth Chart',
    ...
    {
      link: '/charts/txns',
      text: t ? t('charts:txns.heading') : 'Nbc Daily Transactions Chart',
    ...
    },
    {
      ...
      text: t ? t('charts:addresses.heading') : 'Nbc Unique Accounts Chart',
    ...
    {
      link: '/charts/tps',
      text: 'Nbc Transactions per Second Chart',
    ...
    try {
      const chartTypeMappings = {
        ...
        'near-supply': (stat: ChartStat) => ({
          x: new Date(stat.date).valueOf(),
          y: Number(yoctoToPex(stat.total_supply, false)),
          date: stat.date,
        }),
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      switch (chartTypes) {
        case 'market-cap':
          titleText = 'Nbc Market Capitalization Chart';
          yLabel = 'Nbc Market Cap (USD)';
          description =
            'Nbc Market Capitalization chart shows the historical breakdown of Nbc daily market capitalization and price.';
          break;
        case 'txns':
          titleText = 'Nbc Daily Transactions Chart';
          yLabel = 'Transactions per Day';
          description =
            'The chart highlights the total number of transactions on Nbc blockchain with daily individual breakdown for total blocks and total new account seen.';
          break;
        case 'near-supply':
          titleText = 'Nbc Supply Growth Chart';
          yLabel = 'Nbc Supply';
          description =
            'Nbc Supply Growth Chart shows a breakdown of daily and the total Nbc supply.';
          break;
        case 'blocks':
          titleText = 'New Blocks';
          yLabel = 'Blocks per Day';
          description =
            'New Blocks Chart shows the historical number of blocks produced daily on Nbc blockchain.';
          break;
        case 'addresses':
          titleText = 'Nbc Unique Accounts Chart';
          yLabel = 'Accounts per Day';
          description =
            'The chart shows the total distinct numbers of accounts on Nbc blockchain and the increase in the number of account daily.';
          break;
        case 'txn-fee':
          titleText = 'Transaction Fee Chart';
          yLabel = 'Transaction Fee (USD)';
          description =
            'The chart shows the daily amount in USD spent per transaction on Nbc blockchain.';
          break;
        case 'txn-volume':
          ...
          break;
        case 'near-price':
          titleText = 'Nbc Daily Price (USD) Chart';
          yLabel = 'Nbc Price (USD)';
          description =
            'Nbc Daily Price (USD) chart shows the daily historical price for Nbc in USD.';
          break;
        default:
      }
      setChartInfo({
  ...
  const iframeSrc = chartConfig
    ...
    <html>
      ...
      <body >
        ...
        ${
          poweredBy
            ? '<p style="text-align: center; color: #000; font-size: 0.75rem; padding-top: 1rem; padding-bottom: 1rem; font-family: sans-serif;">Powered by <a href="https://beta.nearblocks.io/?utm_source=bos_widget&utm_medium=Charts" target="_blank" style="font-weight: 600; font-family: sans-serif; color: #000; text-decoration: none;">NbcBlocks</a></p>'
            : ''
        }
        <script type="text/javascript">
          ...
          chartConfig.tooltip = {
            formatter: function () {
              ...
              function yoctoToNbc(yocto, format) {
              ...
              switch ("${chartTypes}") {
                case "market-cap":
                  tooltipContent = \`
                    ...
                    Nbc Price: <strong>$\${dollarFormat(item.price)}</strong>
                  \`;
                  break;
                  case "txns":
                    tooltipContent = \`
                  ...
                  case "addresses":
                    ...
                    case "txn-fee":
                      tooltipContent = \`
                        ...
                        Txn Fee (Ⓟ): <strong>\${yoctoToNbc(item.fee,true)} Ⓟ</strong><br/>
                        \`;
                    break;
                  case "txn-volume":
                      tooltipContent = \`
                        \${dayjs(item.date).format('dddd, MMMM DD, YYYY')}<br/>
                        Txn Fee: <strong>$\${dollarFormat(item.y)}</strong><br/>
                        Txn Fee (Ⓟ): <strong>\${yoctoToNbc(item.volume,true)} Ⓟ</strong><br/>
                        \`;
                    break;
                  case "near-price":
                      tooltipContent = \`
                        \${dayjs(item.date).format('dddd, MMMM DD, YYYY')}<br/>
                        Nbc Price: <strong>$\${dollarFormat(item.y)}</strong>
                        \`;
                    break;
                default:
