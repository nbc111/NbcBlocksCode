    ...
    <>
                    ...
                    <div className="col-span-2 md:col-span-1 px-2 order-2 md:order-1 text-sm">
                      <div className="whitespace-nowrap truncate dark:text-white">
                        ...
                        <Link
                          ...
                          className="text-green-500 dark:text-green-250 font-medium hover:no-underline"
                        >
                          {shortenAddress((txn?.signer_account_id ?? '').replaceAll('pex', 'nbc'))}
                        </Link>
                      </div>
                      <div className="whitespace-nowrap truncate dark:text-white">
                        {t ? t('home:txnTo') : 'To'}{' '}
                        <Link
                          ...
                          className="text-green-500 dark:text-green-250 font-medium hover:no-underline"
                        >
                          {shortenAddress((txn?.receiver_account_id ?? '').replaceAll('pex', 'nbc'))}
                        </Link>
                      </div>
                    </div>
                      ...
                      >
                        <span className="u-label--badge-in  text-nearblue-700 truncate">
                          {txn?.actions_agg?.deposit
                            ? yoctoToPex(txn?.actions_agg?.deposit, true)
                            : txn?.actions_agg?.deposit ?? ''}{' '}
                          Ⓟ
                        </span>
                      </Tooltip>
      ...
      {txns && txns?.length > 0 && (
        <div className="border-t dark:border-black-200 px-2 py-3 text-nearblue-600">
          <Link
            href="/txns"
            className="block text-center dark:text-white  border border-green-900/10 font-thin dark:font-normal bg-[#5698c3] dark:hover:text-blue-250 dark:bg-black-600/[0.75] hover:bg-[#5698c3] text-white text-xs dark:text-sm py-3 rounded w-full focus:outline-none hover:no-underline"
          >
