import { formatWithCommas, yoctoToPex } from '@/utils/libs';
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: DelegatorInfo) => (
        <span>
          {formatWithCommas(
            Number(yoctoToPex(row.staked_balance, false)).toFixed(0),
          ) + '  Ⓟ'}
        </span>
      ),
      tdClassName:
    ...
    },
    {
      ...
      cell: (row: DelegatorInfo) => (
        <span>
          {formatWithCommas(
            Number(yoctoToPex(row.unstaked_balance, false)).toFixed(0),
          ) + '  Ⓟ'}
        </span>
      ),
      tdClassName:
