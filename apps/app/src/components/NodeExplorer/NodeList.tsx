...
const NodeList = ({ data, totalSupply, latestBlock, error }: any) => {
  ...
  const TotalSupply = totalSupply
    ? yoctoToPex(totalSupply?.stats[0].total_supply || '0', false)
    : '';
        ...
        <>
          <button>
            ...
            >
              ...
              >
                {row?.accountId?.includes('pex') ? row.accountId.replace(/pex/g, 'nbc') : shortenAddress(row?.accountId ?? '')}
              </Link>
            </Tooltip>
          </button>
              ...
              <td colSpan={3} className="px-4 py-2">
                <div className="flex flex-wrap text-xs text-left font-semibold text-nearblue-600 dark:text-neargray-10 tracking-wider py-2">
                  <div className="flex uppercase whitespace-nowrap">
                    ...
                    <Tooltip
                      label={
                        'Telemetry is a regular notification coming from the nodes which includes generic information like the latest known block height, and the version of NBC Protocol agent (nearcore).'
                      }
                      className="absolute h-auto max-w-xs bg-black bg-opacity-90 z-10 text-xs text-white px-3 py-2"
                    >
                      <button>
                  ...
                  <div className="flex uppercase whitespace-nowrap">
                    ...
                    <div className="relative">
                      <Tooltip
                        label={
                          <span>
                            NBC Protocol could have multiple implementations, so
                            agent is the name of that implementation, where
                            &quot;near-rs&quot; is &nbsp;
                            <Link
          ...
          {!row?.description && (
            <td colSpan={11} className="px-8">
              <div className="flex justify-center text-sm text-nearblue-600 dark:text-neargray-10 font-medium py-4 ">
                If you are node owner feel free to fill all&nbsp; data &nbsp;to
                promote your own node!
              </div>
            </td>
          )}
        </tr>
